...
function WorkerAdapter ({context, source, controller}) {
  ...
  adapter.node.then((node) => {
    // We don't want to connect the analysis node to the destination,
    // as it would cause the audio to be played twice.
...
// The original file uses ES modules, but for use in a non-module content script,
// we might need to expose it to the window object or use a bundler.
// For now, we leave it as is and will handle loading it as a module.
// The line `export { LoudnessMeter };` from the original file is removed
// because we will handle this differently.
window.LoudnessMeter = LoudnessMeter;